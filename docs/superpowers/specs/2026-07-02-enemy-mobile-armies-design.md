...
- **Concurrent same-target assault:** if an `attackerOwner:'enemy'` battle is already open at the target when another enemy army arrives, it **joins** that battle (reinforce side A via the existing `joinFight`) rather than opening a second — keeping a single outcome. (Deeper multi-army coordination is deferred.)
