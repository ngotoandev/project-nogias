...
- **Parity:** new v4 fixtures (sortie wins / repelled / lethal). Anchor `86e238c1` + all 23 existing fixtures unchanged.
...
- **Player attacks byte-identical** (the `attackerOwner==='player'` outcome branch is today's code verbatim; `buildFightSetup` untouched; opt-in `enemyReclaims` default-off → no sortie in existing fixtures) → anchor `86e238c1` + all 23 existing fixtures frozen.
